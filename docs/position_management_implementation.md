...
## Problem Statement
Implement position-aware decisioning so TradingAgents can ingest open positions, produce structured decisions with stop loss/take profit and confidence percentage, enforce trailing-stop guardrails, and propagate those outputs through run flows and backtesting.

## Selected Options (from Alternatives & Tradeoffs)
1. Decision format: **Structured JSON output** from risk manager (`{decision, stop_loss, take_profit, confidence_pct, rationale}`)
...
### Confidence Percentage Definition
- Add `confidence_pct` (numeric, 0-100) to the final recommendation JSON.
- `confidence_pct` represents the model's confidence, based on confluence across indicators/debates, that the trade will be profitable.
- For long-position recommendations, this confidence specifically estimates the likelihood that price increases enough to be profitable on the next trading day.
...
1. Update trader prompt (`tradingagents/agents/trader/trader.py`):
    ...
    - Require trader output to include a JSON block with decision fields, including `confidence_pct`
...
3. Update risk manager (`tradingagents/agents/managers/risk_manager.py`):
    ...
    - Include `confidence_pct` based on indicator confluence and debate confluence, offering a measurable way to express the degree of confidence the trade will increase in price over the next day (in the case of long entries)
    - Include rationale referencing analyst debate + past memory and the confidence justification
...
1. Replace decision-only extraction in `tradingagents/graph/signal_processing.py` with deterministic structured parsing:
    - Extract JSON block
    - Parse and validate required keys (including `confidence_pct`)
    - Validate `confidence_pct` range is within 0-100
...
1. Update `run.py`:
    ...
    - Display decision + stop_loss + take_profit + confidence_pct when present
...
3. Update `cli/main.py`:
    ...
    - Include stop/take-profit/confidence fields in saved report sections where appropriate
...
4. Extend CSV outputs with at least:
    - `date`, `decision`, `stop_loss`, `take_profit`, `confidence_pct`, `position_status`, `rationale`
...
- Confidence score inconsistency across agents -> define and document a shared confluence rubric for `confidence_pct`
